...
## EXASCALAR PLOT
    ...
    p <- p + scale_colour_manual(name='List',values=c("2015-06-01"="darkred","2013-06-01"="blue"))
...
BoundsTable<-rbind("Top Performance" = TopPerformance, "Top Efficiency" = TopEfficiency, "Low Efficiency"=LowestEfficiency, "Top Exascalar"= HighestExascalar, "Bottom Exascalar" = LowestExascalar)
...
BoundsTable<-rbind("Top Performance" = TopPerformance, "Top Efficiency" = TopEfficiency, "Top Exascalar"= HighestExascalar)
...
```{r, 'date trend eff', echo=FALSE, fig.align="center"}
    ...
    for (i in all_dates) {
        ...
        max_eff<-max(Ex$mflopswatt)
        eff_max_perf<-Ex$mflopswatt[Ex$rmax==max(Ex$rmax)]
#         
...
```{r, 'date trend exa', echo=FALSE, fig.align="center"}
...
#         
    ...
    q<- q + ggtitle("Exascalar Trend \n Top Efficiency (Green500) and Top Performance (Top500)")
...
```{r, echo=FALSE, fig.align='center', fig.height=4.25, fig.width=8, warning=FALSE}

## EXASCALAR PLOT


    ## Make sure both dataframes have the same columns
    common_columns <- intersect(names(Jun15), names(Nov14))

    Jun15<-Jun15[, common_columns]
    Nov14<-Nov14[, common_columns]
    
    BigData<-rbind(Nov14,Jun15)
    ## clean up date
    ...
    #p <- p + scale_color_gradientn(colours=rainbow(length(table(BigData$coressocket.t))))
...
## EXASCALAR PLOT
    ...
    ## CREATE EXASCALAR PLOT
    ## take the log of the data points explicitly rather than use a log-scale command
    p <- ggplot(BigData, aes(x=log10(mflopswatt), y=log10(rmax*10^3), color=totalcores.t))
    ...
    ## Define plot points and symbols
    ## alpha = 0.8 helps visually recognize where points overlap
    p <- p + geom_point(alpha=0.6, size=3, pch=20)
    p <- p + scale_color_gradient(low="blue", high="darkorange", na.value="grey50")
    #p <- p + scale_color_brewer(palette="Dark2")
...
```{r, echo=FALSE, fig.align='center', fig.height=6.0, fig.width=8, warning=FALSE}
...
## EXASCALAR PLOT
    ...
    ## CREATE EXASCALAR PLOT
    ## take the log of the data points explicitly rather than use a log-scale command
    p <- ggplot(BigData, aes(x=log10(mflopswatt), y=log10(rmax*10^3), color=log10(totalcores.t)))
    ...
    p <- p + scale_color_gradient(low="blue", high="red", na.value="grey50")
    #p <- p + scale_colour_manual(values=rainbow(length(unique(BigData$acceleratorcoprocessor.g))))
...
## EXASCALAR PLOT
    ...
    p <- p + scale_colour_manual(values=rainbow(length(unique(BigData$processorgeneration.t))))
...
## EXASCALAR PLOT
    ...
    BigData$date<-substring(BigData$date, 1, 7)
    ## clean up processorgeneration
    #BigData$processorgeneration.t<-gsub("Intel Xeon", "Xeon", BigData$processorgeneration.t)
    #BigData$processorgeneration.t<-gsub("Xeon", "Intel Xeon", BigData$processorgeneration.t)
    
    BigData$acceleratorcoprocessor.g<-gsub(".*Phi.*", "Xeon Phi", BigData$acceleratorcoprocessor.g, ignore.case=TRUE)
    
    BigData$acceleratorcoprocessor.g<-gsub(".*Firepro.*", "AMD Firepro", BigData$acceleratorcoprocessor.g, ignore.case=TRUE)
